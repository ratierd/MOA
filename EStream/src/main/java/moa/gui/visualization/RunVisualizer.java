...
public class RunVisualizer implements Runnable, ActionListener, ClusterEventListener{
    ...
    /* panel for bitset visualization */
    private BitSetPanel m_bitsetpanel;
    ...
    public RunVisualizer(ClusteringVisualTab visualPanel, ClusteringSetupTab clusteringSetupTab){
        ...
        m_bitsetpanel = new BitSetPanel();
        visualPanel.add(m_bitsetpanel);
        m_bitsetpanel.setVisible(false);
    ...
    private void evaluateClustering(Clustering found_clustering, Clustering trueClustering, ArrayList<DataPoint> points, boolean algorithm0){
        ...
        for (int i = 0; i < m_measures0.length; i++) {
            if(m_measures0[i].getClass().toString().contains("BitSetMeasure") || m_measures1[i].getClass().toString().contains("BitSetMeasure")) {
                if (m_measures0[m_evalPanel.getSelectedRadioButtonIndex()].getClass().toString().contains("BitSetMeasure")) {
                    m_bitsetpanel.setTable(found_clustering);
                    if(!m_bitsetpanel.isVisible()) {
                        m_bitsetpanel.setVisible(true);
                    }
                }
                else {
                    m_bitsetpanel.setVisible(false);
