 ...
 */
package moa.clusterers.sedstream;
 ...
 * @author david
 */
public final class SEDStream extends AbstractClusterer {
    ...
    public FloatOption overlapThresholdOption = new FloatOption("overlapThreshold", 'o', "Overlap Threshold", 0.7);
    private double overlapThreshold;

    private AutoExpandVector<FadingClusterWithBitVector> clusters;
    private FadingClusterWithBitVector sdCluster;
    ...
    public void resetLearningImpl() {
        ...
        overlapThreshold = overlapThresholdOption.getValue();
        clusters = new AutoExpandVector<>();
        initialized = false;
    }
    
    int instanceCpt = 0;
    Instance incomingDatapoint;
    ...
    public void trainOnInstanceImpl(Instance instance) {
        ...
        if (!this.initialized) {
            ...
            if(flagActiveClusters()) {
                computeDimensionsOverlap(sdCluster.sd(), clusters);
            }
            addDatapoint();
        }
    }
     ...
     */
    private void init() {
        clusters.add(new FadingClusterWithBitVector(incomingDatapoint));
        sdCluster = new FadingClusterWithBitVector(incomingDatapoint);
    }
    ...
    private void fadingAll() {
        ...
        for (int i = clusters.size() - 1; i >= 0; i--) {
            ...
            sdCluster.fading(Math.pow(0.5, decayRate * constDiffTime));
            sdCluster.addDatapoint(incomingDatapoint);
        }
    }
    ...
    /**
     * Select and project dimensions of each cluster
     * 
     * @param globalSd
     * @param clusters 
     */
    private void computeDimensionsOverlap(double[] globalSd, AutoExpandVector<FadingClusterWithBitVector> clusters) {
        AutoExpandVector<FadingClusterWithBitVector> normalizedClusters = FadingClusterWithBitVector.normalize(globalSd, clusters);
        
        int nbDimension = normalizedClusters.get(0).getDimension();
        int countActive = 0;
        for (FadingClusterWithBitVector fc : clusters) {
            if (fc.isCluster()) {
                countActive++;
        ...
        int overlapThresholdValue = (int) Math.ceil(overlapThreshold * countActive);
        
        //* fcId, dimId, overlapCount, radii *//
        ArrayList<Tuple4<Integer, Integer, Integer, Double>> radii = new ArrayList<>();
        
        for (int i = 0; i < clusters.size(); i++) {
            if (clusters.get(i).isCluster()) {
                clusters.get(i).setBitVector(new BitVector(nbDimension));
                normalizedClusters.get(i).setBitVector(new BitVector(nbDimension));
                
                double[] center1 = normalizedClusters.get(i).getCenter();
                double[] sd1 = normalizedClusters.get(i).sd();
                int[] overlapCount = new int[nbDimension];
                for (int j = 0; j < overlapCount.length; j++) {
                    overlapCount[j] = 0;
                }
                
                for (int j = 0; j < clusters.size(); j++) {
                    if (clusters.get(j).isCluster() && i != j) {
                        double[] center2= normalizedClusters.get(j).getCenter();
                        double[] sd2 = normalizedClusters.get(j).sd();
                        
                        for (int dim = 0; dim < nbDimension; dim++) {
                            double distance = Math.abs(center1[dim] - center2[dim]);
                            if (distance < 0.75 * (sd1[dim] + sd2[dim])) {
                                overlapCount[dim]++;
                            }
                        }
                    }
                ...
                for (int dim = 0; dim < nbDimension; dim++) {
                    if (overlapCount[dim] <= overlapThresholdValue) {
                        radii.add(new Tuple4(i, dim, overlapCount[dim], sd1[dim]));
                    }
                }
            }
        }
        
        Comparator<Tuple4<Integer, Integer, Integer, Double>> SORT_RADII = new Comparator<Tuple4<Integer, Integer, Integer, Double>>() {
            @Override
            public int compare(Tuple4<Integer, Integer, Integer, Double> o1, Tuple4<Integer, Integer, Integer, Double> o2) {
                if (o1._4() < o2._4()) {
                    return -1;
                }
                else {
        ...
        FadingClusterWithBitVector.deleteZeroBitVectorCluster(normalizedClusters);
    ...
    private void addDatapoint() {
        if (!addToClosestCluster(incomingDatapoint)) {
            clusters.add(new FadingClusterWithBitVector(incomingDatapoint));
        }
    }
