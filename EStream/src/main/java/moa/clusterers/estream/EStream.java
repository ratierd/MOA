 ...
 */
package moa.clusterers.estream;
...
import moa.cluster.FadingCluster;
 ...
 * @author david
 */
public final class EStream extends AbstractClusterer {
    ...
    private AutoExpandVector<FadingCluster> clusters;
    ...
    private void init(Instance instance) {
        clusters.add(new FadingCluster(instance));
    }
        ...
        for (int i = clusters.size() - 1; i >= 0; i--) {
            if(clusters.get(i).isCluster()) {
                if (clusters.get(i).checkSplit()) {
                    FadingCluster lowerFc = clusters.get(i).split();
    ...
    private void mergeOverlapClusters() {
        for (int i = 0; i < clusters.size(); i++) {
            FadingCluster fc1 = clusters.get(i);
            if(fc1.isCluster()) {
                for (int j = i + 1; j < clusters.size(); j++) {
                    FadingCluster fc2 = clusters.get(j);
                    if(fc2.isCluster()) {
                        if (fc1.isOverlapping(fc2, mergeThreshold)) {
                            FadingCluster testSplit = new FadingCluster(fc1);
                            testSplit.merge(fc2);
    ...
    private void limitMaximumClusters() {
        AutoExpandVector<FadingCluster> targets = new AutoExpandVector<>();
     ...
     *
     */
    private void mergeClosestClusterPair(AutoExpandVector<FadingCluster> targets) {
        double distance = Double.NaN;
        FadingCluster resFc1 = null;
        FadingCluster resFc2 = null;
        double weight = Double.NaN;
        
        for (int i = 0; i < targets.size(); i++) {
            FadingCluster fc1 = targets.get(i);
            for (int j = i + 1; j < targets.size(); j++) {
                FadingCluster fc2 = targets.get(j);
                double cDist = FadingCluster.getCenterDistance(fc1, fc2);
    ...
    /**
     * Flag clusters depending on their weight
     */
    private void flagActiveClusters() {
        for (FadingCluster fc : clusters) {
    ...
    private boolean addToClosestCluster(Instance instance) {
        double distance = Double.NaN;
        FadingCluster res = null;
        double weight = Double.NaN;
        for (FadingCluster fc : clusters) {
            if (fc.isCluster()) {
                double cDist = FadingCluster.getNormalizedDistance(fc, instance);
                if ((cDist < distance || Double.isNaN(distance))) {
                    distance = cDist;
                    res = fc;
                    weight = fc.getWeight();
